...
Backend ограничивает размер файла, количество этапов и критериев, длину каждого поля и допустимые идентификаторы. После валидации хранится неизменяемый `scenarioSnapshot`; raw Markdown не логируется. Готовые сценарии лежат в `backend/src/main/resources/scenarios/*.md` и проходят ту же валидацию, что и пользовательский импорт.
...
- На хакатоне сценарии — versioned resources и session snapshot, а не полноценный редактор и каталог методистов.
